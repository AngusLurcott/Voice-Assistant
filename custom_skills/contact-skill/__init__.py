...
class Contact(MycroftSkill):
    ...
    # Method to obtain Emergency contact number of user
    @skill_api_method
    def get_emergency_contact_number(self):

        try:
            contacts = self.db.child("contacts/{}".format(self.user_id)).order_by_child("emergency").equal_to(True).get().val()
            values = list(contacts.values())
            phone_number = 0
            for s in range(len(values)):
                phone_number = values[s]['number']
            self.speak_dialog(phone_number)
        except:
    ...
    #Method to obtain emergency contact email of user
    @skill_api_method
